...
var checksum = 0
for line in lines {
    let numbers = line.components(separatedBy: .whitespaces).flatMap({ Int($0) })
    let max = numbers.max()!
    let min = numbers.min()!
    checksum += (max - min)
}
...
outer: for line in lines {
    let numbers = line.components(separatedBy: .whitespaces).flatMap({ Int($0) }).sorted(by: >)
    for (index, numerator) in numbers.enumerated() {
        for denominator in numbers[(index+1)...] {
            if numerator % denominator == 0 {
                outputSum += numerator/denominator
                continue outer
            }
        }
    }
